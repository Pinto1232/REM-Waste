...
export function SkipsList({
  ...
  const {
    ...
    searchParams && (searchParams.postcode || searchParams.area) ? searchParams : null
  );

  if (!searchParams || (!searchParams.postcode && !searchParams.area)) {
    return (
  ...
  const formatPrice = (priceBeforeVat: number, vat: number) => {
    try {
      if (typeof priceBeforeVat !== 'number' || isNaN(priceBeforeVat) || priceBeforeVat < 0) {
        console.warn('Invalid price before VAT:', priceBeforeVat);
        return 'Price unavailable';
      }

      if (typeof vat !== 'number' || isNaN(vat) || vat < 0) {
        console.warn('Invalid VAT rate:', vat);
        return 'Price unavailable';
      }
  ...
  const handleSkipSelect = (skip: Skip) => {
    try {
      if (!skip || typeof skip !== 'object') {
        console.error('Invalid skip data provided to handleSkipSelect:', skip);
        return;
      }

      if (!skip.id) {
        console.error('Skip missing required ID:', skip);
        return;
      }

      if (onSkipSelect && typeof onSkipSelect === 'function') {
        onSkipSelect(skip);
      }
    } catch (error) {
      console.error('Error handling skip selection:', error);
    }
  };
  ...
  const filteredSkips =
    skips?.filter(skip => {
      try {
        if (!skip || typeof skip !== 'object') {
          console.warn('Invalid skip object in filter:', skip);
          return false;
        }

        if (
          typeof skip.id === 'undefined' ||
          typeof skip.size === 'undefined' ||
          typeof skip.price_before_vat === 'undefined' ||
          typeof skip.vat === 'undefined'
        ) {
          console.warn('Skip missing required properties:', skip);
          return false;
        }
        ...
        let totalPrice = 0;
        try {
          if (typeof skip.price_before_vat === 'number' && typeof skip.vat === 'number') {
            totalPrice = skip.price_before_vat * (1 + skip.vat / 100);
          }
        } catch (error) {
          console.warn('Error calculating total price for skip:', skip.id, error);
        }

        if (searchFilters.searchTerm) {
          const searchLower = searchFilters.searchTerm.toLowerCase();
          const matchesSearch =
            (skip.size && skip.size.toString().includes(searchLower)) ||
            formatPrice(skip.price_before_vat, skip.vat).toLowerCase().includes(searchLower) ||
            (skip.hire_period_days && skip.hire_period_days.toString().includes(searchLower)) ||
            (typeof skip.allowed_on_road === 'boolean' &&
        ...
        if (searchFilters.sizeRange !== 'all') {
          ...
          if (typeof size !== 'number' || isNaN(size)) {
            console.warn('Invalid size for skip:', skip.id, size);
            return false;
          }
        ...
        if (searchFilters.priceRange !== 'all') {
          if (totalPrice <= 0) {
            console.warn('Invalid total price for skip:', skip.id, totalPrice);
            return false;
          }
        ...
        if (searchFilters.roadLegal !== 'all') {
          if (typeof skip.allowed_on_road !== 'boolean') {
            console.warn('Invalid allowed_on_road value for skip:', skip.id, skip.allowed_on_road);
            return false;
          }
        ...
        console.error('Error filtering skip:', skip?.id || 'unknown', error);
        return false;
      }
    }) || [];
  ...
  try {
        ...
        {skips && skips.length > 0 && filteredSkips.length > 0 && (
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 px-4 sm:px-0'>
            {filteredSkips.map(skip => {
              if (!skip || !skip.id) {
                console.warn('Skipping invalid skip in render:', skip);
                return null;
              }
                ...
                >
                      ...
                      <div className='flex items-center'>
                        ...
                        <div className='ml-3'>
                          <h3 className='text-lg font-bold text-white'>
                            {skip.size || 'Unknown'} Yard Skip
                          </h3>
                          <p className='text-slate-300 text-xs'>
                            {skip.area || 'Unknown area'} • {skip.postcode || 'Unknown postcode'}
                          </p>
                        </div>
                      </div>
                      {selectedSkipId === skip.id && (
                  ...
                  <div className='p-4'>
                    ...
                    <div className='mb-4'>
                      ...
                      <div className='text-xs text-slate-400'>Including {skip.vat || 0}% VAT</div>
                    </div>
                    ...
                    <div className='grid grid-cols-2 gap-3 mb-4'>
                      <div className='bg-slate-700/30 rounded-lg p-2 text-center'>
                        ...
                        <div className='text-sm font-semibold text-white'>
                          {skip.hire_period_days || 'N/A'} days
                        </div>
                      </div>
                      <div className='bg-slate-700/30 rounded-lg p-2 text-center'>
                        <div className='text-xs text-slate-400 mb-1'>Heavy Waste</div>
                        <div className='text-sm font-semibold text-white'>
                          {typeof skip.allows_heavy_waste === 'boolean' ? (
                    ...
                    <div className='space-y-2 mb-4'>
                      <div className='flex items-center text-xs'>
                        <svg
                          ...
                          viewBox='0 0 20 20'
                        >
                          {typeof skip.allowed_on_road === 'boolean' && skip.allowed_on_road ? (
                            <path
                              fillRule='evenodd'
                      ...
                      {typeof skip.forbidden === 'boolean' && !skip.forbidden && (
                        <div className='flex items-center text-xs'>
                          <svg
        ...
        {skips && skips.length === 0 && !isLoading && (
          <div className='text-center py-8'>
            ...
            <p className='mt-1 text-sm text-gray-500'>
              Try searching with a different postcode or area.
            </p>
          </div>
        )}
      </div>
    ...
    console.error('Critical error in SkipsList component:', error);

    return (
      <div className='w-full'>
        <div className='bg-red-50 border border-red-200 rounded-md p-4 mb-6 mx-4 sm:mx-0'>
          <div className='flex'>
            <div className='flex-shrink-0'>
            ...
            <div className='ml-3'>
              ...
              <p className='mt-1 text-sm text-red-700'>
                Something went wrong while displaying the skips list. Please refresh the page and
                try again.
              </p>
              <button
